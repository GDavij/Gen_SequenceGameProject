...
function message_player (txt:string) {
    const msg_interface = document.getElementById("MessageUser") as HTMLDivElement
...
const render_color_seq = (color:number) => {// Subdependency from 'render_sequence'
    const button = document.getElementById("SequenceOut") as HTMLButtonElement
    switch(color){
        case 0 : // Red
...
function render_sequence (sequence:Array<any>) {
    let count = 0
...
function generate_game_screen (instance:number) {
    const screen = document.querySelector("#Game") as HTMLDivElement
...
const render_blink_color = (color:number) => {// Subdependency from 'render_sequence'
    const button_ids = ['Red','Yellow','Blue','Green']
    const button = document.getElementById(button_ids[color]) as HTMLButtonElement
    switch(color){
        case 0 : // Red
